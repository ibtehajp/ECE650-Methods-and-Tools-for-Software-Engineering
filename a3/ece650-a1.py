...
def check_coordinates(line1):
    ...
    if bool(re.search(r"\)\(", line1)) == True or bool(re.search(r"\)\s*\,\s*\(", line1)) == True: 
        raise Exception("1Coordinate formatting is incorrect") # checking for no space between coordinates of comma between coordinate set

    if bool(re.search(r"\(\s*\(+", line1)) == True or bool(re.search(r"\)\s*\)+", line1)) == True:
        raise Exception("2Coordinate formatting is incorrect") # checking for more than one same consecutive brackets

    if bool(re.search(r"\-\s*\-+", line1)) == True:
        raise Exception("3Coordinate formatting is incorrect") # checking for more than one negative sign

    if bool(re.search(r"\)\s*[a-zA-Z0-9]+", line1)) == True or bool(re.search(r"[a-zA-Z]+\s*\)", line1)) == True:
        raise Exception("4Coordinate formatting is incorrect") # checking for alphabets with open bracket. In some case numbers were also considered 

    if bool(re.search(r"\(\s*[a-zA-Z]+", line1)) == True or bool(re.search(r"[a-zA-Z0-9]+\s*\(", line1)) == True:
        raise Exception("5Coordinate formatting is incorrect") # checking for alphabets with open bracket. In some case numbers were also considered 

    if bool(re.search(r"\)\s*[@''`.,_!#$%^&*<>?/\|}{~:-]+", line1)) == True or bool(re.search(r"[@''`.,_!#$%^&*<>?/\|}{~:-]+\s*\)", line1)) == True:
        raise Exception("6Coordinate formatting is incorrect") # checking for special characters with close brackets

    # if bool(re.search(r"\(\s*[@''`.,_!#$%^&*<>?/\|}{~:-]+", line1)) == True or bool(re.search(r"[@''`.,_!#$%^&*<>?/\|}{~:-]+\s*\(", line1)) == True:
    #     raise Exception("7Coordinate formatting is incorrect") # checking for special characters with open brackets
...
def check_command(cmd): 
    if cmd != 'add' and cmd != 'gg' and cmd != 'rm' and cmd != 'mod' and cmd != 'quitquit':
...
def edge_print(ar=[]): # it will print the edge list by using the order list of the vertex
    ...
    sys.stdout.write("E {")
    for i in range(0, len(ar)-1):
        sys.stdout.write('<'+str(ar[i][0])+','+str(ar[i][1])+'>,')
    if len(ar) != 0:
        i = len(ar)-1
        sys.stdout.write('<'+str(ar[i][0])+','+str(ar[i][1])+'>')
    sys.stdout.write('}\n')  
